...
for (n in nms) {
  ...
  tmp <- list.files(
    dir_qc, pattern = glue::glue("{n}X?-cDNA.*\\.bmp"), full.names = T
  )
  if (length(tmp) == 0) {
    qc_cdna[n] <- png_na 
  } else {
...
cmd <- glue::glue("cp '{ref_csv}' {dir_out}/tables/feature_ref.csv")
system(cmd)
ref <- vroom::vroom(ref_csv, delim = ",", col_types = vroom::cols())
```
