...
pkgs <- c("glue", "stringr", "dplyr", "vroom", "DiagrammeR", "openxlsx", "tidyr", "pander")
for (pkg in pkgs) suppressPackageStartupMessages(library(pkg, character.only = T))


if (!dir.exists(glue("{dir_out}/"))) dir.create(glue("{dir_out}/"), recursive = T)
if (!dir.exists(glue("{dir_out}/summaries/"))) dir.create(glue("{dir_out}/summaries/"), recursive = T)
if (!dir.exists(glue("{dir_out}/tables/"))) dir.create(glue("{dir_out}/tables/"))
...
nms <- sapply(hs, function(x) {
  stringr::str_extract(x, "cellranger_out_.*") |>
    ...
    stringr::str_replace_all("cellranger_out_", "")
})

t_c <- lapply(as.list(1:length(ts)), function(x) {
  vroom(ts[x], delim = ",", col_types = cols()) |>
    mutate(Sample_ID = nms[x]) |>
    dplyr::select(Sample_ID, everything())
}) |>
  bind_rows()

t_l <- vector("list", length = ncol(t_c)) |>
  setNames(colnames(t_c))
for (n in names(t_l)) {
  t_l[[n]] <- t_c[, n]
}
...
write.xlsx(t_cf, glue("{dir_out}/tables/summary.xlsx"))

for (i in 1:length(hs)) {
  cmd <- glue("cp '{hs[i]}' {dir_out}/summaries/web_summary_{nms[i]}.html")
  system(cmd)
}
hs_f <- list.files(glue("{dir_out}/summaries/"), recursive = T, pattern = "web_summary.*\\.html", full.names = T)

qc_library <- qc_cdna <- vector("character", length(nms)) |>
  setNames(nms)
for (n in nms) {
  tmp <- list.files(
    dir_qc, pattern = glue(".*{n}X?\\.bmp"), full.names = T
  )
  if (length(tmp) == 0) {
    qc_library[n] <- png_na 
  } else {
  ...
  tmp <- list.files(
    dir_qc, pattern = glue("{n}X?-cDNA.*\\.bmp"), full.names = T
  )
  if (length(tmp) == 0) {
    qc_cdna[n] <- png_na 
  } else {
...
emphasize.italics.cells(which(t_cf == c("Fail", "Risk of Doublet", "Suggest to Increase Sequecing Depth", "Acceptable"), arr.ind = T))
emphasize.strong.cells(which(t_cf == c("Fail", "Risk of Doublet", "Suggest to Increase Sequecing Depth", "Acceptable"), arr.ind = T))
...
for (i in 1:length(hs_f)) {
  cat(glue("* **{nms[i]}**样本，详见[链接](summaries/{basename(hs_f[i])})"))
  cat("\n\n\n\n")
  cat(
    glue(
      '<iframe height="800" width="100%" src="\
      summaries/{basename(hs_f[i])}" frameborder="0" allowfullscreen>
</iframe>'
    )
  )
  cat("\n\n\n\n")
